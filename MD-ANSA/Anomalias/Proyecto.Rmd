---
title: "Parte práctica: Anomalías"
...
---
 ...
 Los trozos de código que usted ha de completar (los indicados con COMPLETAR) sí debe mostrarlos en el documento final.
...
Vamos a usar el conjunto de datos ``wine``, un dataset orientado a la clasificación multiclase de diferentes vinos en base a 13 atributos. Estos atributos miden distintos componentes químicos, siendo los siguientes:
...
Todos las variables son numéricas y miden la cantidad encontrada de cada componente.
En nuestro caso ignoraremos la clase de prediccion y las etiquetas de valores anómalos proporcionadas en el dataset.

Es necesario mencionar que no cada vino distinto (cada instancia) únicamente viene identificada por su posición en la tabla. Es decir, no tenemos un nombre o algún identificador para referenciarla.
...
Esto implica que en los apartados con la etiqueta ``# COMPLETAR`` se incluirá el cálculo de los nombres pero aparecerá su salida comentada para no repetir.
...
Todas contienen valores reales (con variedad) y sin valores nulos, por lo que no eliminamos ninguna más
...
De cara a elegir aquellas variables, mostramos histograma de cada una:
```{r}
par(mfrow = c(2,3))
...
En general la mayoría de variables parecen seguir distribuciones no muy raras, excepto en ``ÒD280`` que parece ser bimodal.

De cara a elegir una de estas variables, y por tener un criterio más de desempate, aplicamos un test de normalidad a cada una.
...
Notamos que en las tres gráficas los outliers se ubican alejados de la nube de puntos, tal y como habíamos visto, 3 puntos por arriba del eje de las abscisas y 1 por debajo.
...
Para el resto, vemos que más o menos habíamos acertado en el resto, solo la 11 parece tambier estar un tanto alejada de su media en la columna ``Proantho``
...
Primeramente, podemos ver de forma gráfica que la distribución de la variable elegida:
```{r}
ajusteNormal = fitdist(columna , "norm")
denscomp(ajusteNormal,  xlab = nombre.columna)
```

Que vemos que se asemeja 

### Test de Grubs

Más adelante se aplicará el test de normalidad sobre la variable sin el outlier.
Antes, basándonos en el gráfico anterior y suponiendo que la distribución es normal, podemos hacer el test de Grubs sobre el valor más alejado de la media:
```{r}
test.de.Grubbs = grubbs.test(columna, two.sided = TRUE)
test.de.Grubbs$p.value
```

Mostramos el valor de la instancia:
```{r}
...
Pasamos los tests de Normalidad al conjunto de datos eliminando previamente el outlier O (instancia 11):
<!-- INCLUIR -->
```{r}
...
Notamos valores extremadamente altos en Mag y Proline, y solo unas pocas variables con outliers.
...
La Proline es el amino ácido más abundante en los vinos y parece variar en base al tipo de fermentación que ha sufrido el vino.
```{r}
...
Quitamos la variable pues parece bimodal, pero mantenemos el resto por no ser demasiado ``raras``
<!-- La 2, 10, 13 -->
...
El p-value supera por poco nuestro nivel de significación, por lo que el test no lo rechaza con significación.
...
No deberíamos por tanto aplicar este método con distancia de Mahalanobis si queremos garantía estadística, pues la condición de que las variables se distribuyan de forma normal era necesaria.
...
Podemos ver que hay 3 puntos fuera de las elipses, 2 considerablemente más alejados que el tercero.
También apreciamos que el uso o no de un método robusto no nos cambia significativamente los resultados.

<!-- QUITAR -->
<!-- Por comparar, mostramos los outliers que se obtenían con estas variables usando el método IQR: -->
<!-- ```{r} -->
<!-- plot_2_colores(datos.num.norm[,c(3,8)], claves.outliers.IQR) -->
<!-- ``` -->
<!-- Tenemos dos puntos igualmente detectados, y diferencias en el resto. Siendo la nube de puntos algo dispersa, parece que el método con distancia  -->


Aplicamos test individual y test múltiple, ajustando el parámetro alpha de forma que corrijamos el FWER
<!-- INCLUIR -->
```{r}
set.seed(2)
...
El test de intersección no nos devuelve ningún outlier, pero el indivual nos saca 4 puntos. Todos estos fueron detectados como outliers por el método IQR en alguna columan, siendo 11 y 73 en más de una.
```{r}
# claves.outliers.IQR.en.mas.de.una.columna
```


Si se cumpliera el requisito de distribución normal multivariante, tendrías garantía estadística en la instancia con mayor distancia Mahalanobis.
...
Apreciando los 4 puntos vistos anteriormente y un segundo conjunto de puntos un poco por debajo. Este conjunto probablemente esté algo alejado de la nube de puntos, pero no sean outliers.
...
Si cumpliéramos el requisito de normalidad, concluiríamos con garantía estadística que el registro 11 rechaza la hipótesis y es un outlier. Este punto ya fue detectado como outlier tanto en una como en varias columnas.


## Visualización de datos con un Biplot

Mostramos visualmente información resumida del conjunto de datos en dos variables, junto a los outliers detectados medianteme alguna columna con IQR:
```{r}
...
Lo primero que debemos notar es que el biplot no explica un alto porcentaje de las varianzas, probablemente por la cantidad de variables, que vuelva difícil explicar la variabilidad de los datos en solo dos componentes. Esto hace que nuestra confianza en los valores mostrados en el biplot no sea muy alta, pues existe una alta variabilidad no mostrada en el gráfico.
Además vemos que una gran cantidad de outliers se ubican con valores altos en alguna columna, alejados del centro. Solo el outlier detectado 11 se aprecia con valores muy inferiores en un conjunto de variables (Ash, Alcohol, Mag...) pero con valores normales en el resto.

También apreciamos que las variables están agrupadas en 3 grupos, con correlaciones positivas y negativas entre ellas.
<!-- INCLUIR -->
```{r}
...
No tenemos la información expresada por las variables que hemos quitado, pero en base a estas optenemos una variabilidad explicada del 14.8+54.8=69.6%, y podemos intentar explicar de nuevo los resultados.
...
Concluímos diciendo que se aprecia enormemente en este dataset que la información proporcionada por un biplot debe ser una ayuda al científico, y un método único en el que basarse para la detección de outliers.

## Métodos basados en distancias: LOF

La selección de un buen número de vecinos es vital para los métodos basados en vecinos más cercanos. Un valor demasiado grande puede hacernos detectar como outliers nubes con bajo número de puntos, mientras que uno demasiado alto.
El método LOF basado en densidad paliará en parte estos problemas, pero sigue siendo necesaria una selección cuidadosa.

En base a las gráficas vistas a lo largo de la práctica, concretamente las de la sección Gráficos REVISAR, vemos a la mayoría de puntos generalmente agrupados en la misma nube, no apreciando multiples agrupaciones diferentes (debemos tener cuidado con esto puesto que así no tenemos en cuenta los outliers con valores anómalos en más de 2 columnas). En base a este razonamiento, elegimos un número de vecinos medianamente alto, de 7.
```{r}
num.vecinos.lof = 7
...
Vemos un valor con un alto grado de anomalía y dos conjuntos que lo siguen, el primero formado por 6 instancias y el segundo por 2.
...
La que obtiene más es la 73, que también había sido detectada por los métodos anteriores. Visualizando sus datos normalizados, vemos que solo tiene datos alejados en 2 columnas (3 si consideramos Alcalinity).
...
Analizamos el registro con mayor score de LOF (73) en diagramas de dispersión (puesto que la figura es excesivamente grande con todas las variables, seleccionamos aquellas en las que continee valores extremos)
```{r}
...
Las correlaciones las vimos anteriormente, pero se hace notoria la relación Phenols-Flavanoids, con la instancia más baja de lo que sería esperado si siguieramos una línea de regresión sobre esos datos.
Con Ash-Alcalinity ocurre algo similar, pero el punto se ubica alejado del resto, pero sin contradecir la tendencia.
...
Como habíamos mencionado, contiene inusualmente altos en varias variables, pero se sigue ubicando en una zona con gran densidad.
...
<!-- SEGUIR CON EL ANÁLISIS ANTERIOR -->
...
Intancias que también habíamos obtenido con otros métodos.
...
No mostramos el biplot con el número reducido de columnas pues la separación en clústers ha hecho uso de todas.
...
Respcecto a los del grupo 2, este es un clúster bastante disperso en este biplot (recordamos que la varianza explicada es baja), pero los outliers están alejados de su centroide.
...
Vemos que se ha mantenido un clúster con la misma agrupación que con k=3, y los otros dos se han dividido en dos partes.
Hacemos notar que el outlier 110, que hubiera pertenecido al clúster 5, al haberse este particionado ya no lo tenemos como uno de los 5 puntos con mayor distancia, puesto que el clúster se ha acercado a su posición.
Tenemos también dos nuevos puntos (25 y 48), el 25 lo vemos cerca de su clúster. Es probable que el biplot no nos muestre la información que lo convierta en un valor anómalo.
Respecto al 48, lo tenemos muy cerca del centro del biplot. No es el único punto en esta zona, aunque está ligeramente desplazado hacia arriba. Es posible que tenga una combinación anómala, o que una vez más la información del biplot sea insuficiente.
...
La instancias 73 tiene valores extremos en 3 columnas.
La 25 y la 11 en 2.
...
Vemos también que el orden de las distancias de los outliers se corresponde con el número de columnas en las que son valores extremos estas instancias. Solo 25 tiene una distancia menor a 21, pero podemos apreciar que 21 tiene valores no extremos, pero alejados del centro de la distribución en la mayoría de columnas.
...
Calculamos los 5 posibles outliers:
<!-- INCLUIR -->
```{r}
...
Lo primero que notamos es unos clústers totalmente diferentes a los del método K-means. En este caso tenemos una única agrupación a la derecha y 3 clústers en la parte centro-inferior izquierda.
...
Todos tienen valores extremos más de una columna excepto la 62, que tiene cantidades normales de todos los componentes excepto una gran acumulación de Proantho.
...
Volvemos a calcular los outliers LOF aumentando el umbral a 9 puntos, y buscamos los multivarialbes puros:
```{r}
...
A partir del diagrama de cajas apreciamos que aunque no tienen un valor extremo en ninguna de las variables, en la mayoría contienen una cantidad fuera del 1al3 cuartil (REFORMULAR). Esto sea lo que probablemente haga aumentar el score de LOF y presente un alto grado de anomalía.
...
PODEMOS DECIR QUE 11 ES UN VERDADERO OUTLIER, TODOS LOS MÉTODOS LO DETECTAN?
