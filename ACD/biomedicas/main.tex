...
% ?
% Adicionalmente, se indica la posible insuficiencia del modelo en uso forense si no se incluyen detalles a la cara devuelta

% También se podría mencionar como inconveniente esto que menciona explícitamente el paper: "The resulting approximation is an objective probable facial shape, but is also synthetic (i.e., without tex-ture), and therefore needs to be enhanced artistically prior to its use in forensic cases."
...
% Es importante mencionar numérica/cuantitativamente los resultados obtenidos. En base a lo comentado en esta sección no queda claro cómo de bueno o malo es el sistema propuesto.

% Errores medios (en la profundidad del tejido facial?) sobre los datos de prueba
...
% Ojo. La reconstrucción facial, según muchos autores, es más un arte que una ciencia, y su capacidad para identificar personas es más que discutida. Desde este punto de vista, la posibilidad de reconstruir la cara de una persona a partir de sus huesos es bastante debatida. 
