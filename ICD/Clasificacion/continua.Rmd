...
```{r waring=FALSE, results='hide', message=FALSE}
# Mode function from Github
...
```{r}
glmFit <- train(train[,-2], y = train[,2], 
                method = "glm", 
...
```{r}
ggplot(train, aes(x = Lag1, y = Lag2, color=predict(ldaFit))) +
  geom_point(show.legend = F) +
  labs(title="Prediction LDA") +
  theme_light() -> ldaplot

ggplot(train, aes(x = Lag1, y = Lag2, color=classes)) +
  geom_point() +
  labs(title="True values") +
  theme_light() -> trueplot

plot_grid(plotlist=list(ldaplot,trueplot), ncol=2, label_size = 1)
```
...
```{r}
ggplot(train, aes(x = Lag1, y = Lag2, color=predict(glmFit))) +
  geom_point(show.legend = F) +
  labs(title="Prediction GLM") +
  theme_light() -> glmplot

plot_grid(plotlist=list(ldaplot,glmplot,trueplot), ncol=2, label_size = 1)
```
...
```{r}
ggplot(train, aes(x = Lag1, y = Lag2, color=predict(qdaFit))) +
  geom_point(show.legend = F) +
  labs(title="Prediction QDA") +
  theme_light() -> qdaplot

plot_grid(plotlist=list(ldaplot,glmplot,qdaplot,trueplot), ncol=2, label_size = 1)
```
