...
```{r}
# Basic SMOTE
smt <- trainData %>% oversample(method = "SMOTE")

# SMOTE borderline
blsmt <- trainData %>% oversample(method = "BLSMOTE")

# Majority Weighted Minority Oversampling
mwmt <- trainData %>% oversample(method = "MWMOTE")

# PDF estimation based over-sampling
pdf <- trainData %>% oversample(method = "PDFOS")
```
...
```{r}
model.pdf <- learn_model(pdf, ctrl, "PDF ")
cm.pdf <- test_model(testData, model.pdf,"PDF ")
```

Summarize models results
```{r}
models <- list(smt = model.smt, blsmt = model.blsmt, 
               mwmt = model.mwmt, pdf = model.pdf)
results <- resamples(models)
summary(results)
```
...
<!-- ALGO MÁS ?? -->
<!-- CON EL RESTO DE PREPROCESAMIENTOS TAMBIÉN, NO ? -->
...
```{r}
plot_pie(trainData$Class)
plot_pie(smt$Class)
plot_pie(blsmt$Class)
plot_pie(mwmt$Class)
plot_pie(pdf$Class)
```
