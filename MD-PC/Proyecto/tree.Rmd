...
4. Selección de instancias con método ENN tras seleccionar características con método filter de chi2. Previamente realizado un onehotencoding para poder aplicar los métodos pues se basan en distancias. Se habían eliminado variables correladas y las geo2y3 en base a lo mencionado anteriormente.
...
C4.5 crea árboles muy complejos, al contrario que CART

<!-- https://www.quora.com/What-are-the-differences-between-ID3-C4-5-and-CART -->
Datos ruidosos pueden generar overfitting en C4.5

All variables, C4.5, partitions:
- 15% = 62.61% val, 0.6571 f1
- 50% = 63.68% val, 0.6638 f1
- 80% = 64.34% val, 0.6686 f1
- 100% = 64.67% val

---------------------------------------------------------------------------------------------------------------------

Submissions
1. Preprocesamiento v3, sin podas
  - CART: 0.5754
  - C4.5: 0.3364
  
2. Diferentes cosas a cada algoritmo, ver Python
  - CART: 0.5426
  - C4.5: 0.6581 (todas las variables, eliminación de instancias de forma aleatoria, manteniendo desbalanceo de datos, y usando este conjunto separado como validación. De las particiones resultantes, se elige la de 15% por dejarnos un número de instancias suficiente y un descenso en calidad muy pequeño) 
  Se demuestra que para cada algoritmo debemoso llevar un preprocesamiento diferente.

C4.5  
3. Probando C4.5 con mayor conjunto de datos, pero igual que punto 2. Nos mejora, es posible que como hace pruning tras construir el árbol incluso una partición mayor funcione mejor
0.6778

4. El mejor de los anteriores, pero count families como ordinal. Aumento al 95%
Submission: 0.6836
F1: 0.6814
CV accuracy: 64.67%

5. Selección de características (quizás agrupamiento), sin selección de instancias, con porcentaje de validación pequeño
  <!-- Para la siguiente, eliminar puntos ruidosos y mantener todas las variables. Quizás eliminar con tomek links o algún método que reduzca poco los datos y mantenga el balanceo, para luego agrupar las varibles del one-hot-encoding y probar con un gran número de instancias. (Count families está como categórica) -->
  

v7 - Probando con un oversampling con SMOTE seguido de un undersampling con ENN nos desbalancea la clase mayoritaria
(1               75398
3               39579
2               10400)
Buenos resultados en validación (del 5%), pobres en submission 5: 0.5064
POSIBLE OVERFITTING


v8 - Solo con SMOTE, conjuntos balanceados (300000 instancias en total). Random Undersampling de 35%
...
v9 - SMOTE sin one-hot. Sub 7: 0.5703
...
trn_labels <- read_csv("data/preprocessed_train_labels.csv")
...
Separamos un 10% para validar localmente (CAMBIADO)
```{r}
size <- floor(0.75 * nrow(train))
...
Ahora están igualadas todas las clases
```{r}
table(train$labels)
```
...
<!-- $precision -->
<!--         1         2         3  -->
<!--       NaN 1.0000000 0.2240897  -->

<!-- $recall -->
<!--         1         2         3  -->
<!-- 0.0000000 0.9991467 0.8290368  -->
...
```{r}
set.seed(2)
cv.trn <- cv.tree(cart_tree, FUN=prune.misclass)

cv.trn
```

Poda manual (Sería elegir un tamaño de árbol basado en lo anterior)
POR AHORA NO SE PODA, YA ES PEQUEÑO DE POR SÍ
```{r}
prune.trn <- prune.misclass(cart_tree, best = 6)
par(mfrow = c(1,1))
plot(prune.trn)
...
Si no se puede puede que sea por tener muchas hojas (solo si es pequeño)
```{r}
# plot(c45_tree)
```
...
cv_resul <- evaluate_Weka_classifier(c45_tree, numFolds=3)
cv_resul
```
...
# c45_prune <- M5P(labels ~ ., data=train, control=Weka_control(R = TRUE))
#  
# c45_tree$classifier$toSummaryString()
# 
# head(train)
...
```{r}
# cv_resul <- evaluate_Weka_classifier(c45_prune, numFolds=10)
# cv_resul
```
...
El desbalanceo parece estar afectando a la clasificación, la 1 se etiqueta más que la 3
```{r}
# table(cart_test)
table(c45_test)
```

Pero sigue la tendencia de el conjunto de validación
```{r}
# table(cart_predict)
table(c45_predict)
table(val$labels)
...
# buildings_ids %>% 
#   bind_cols(cart_test) %>% 
#   write_csv("./out/cart.csv")

buildings_ids %>% 
  bind_cols(c45_test) %>% 
  write_csv("./out/c45.csv")
```
