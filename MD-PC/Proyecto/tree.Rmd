...
```{r}
trn_values <- read_csv("data/v12 - preprocessed_train.csv")
trn_labels <- read_csv("data/v12 - preprocessed_train_labels.csv")
...
Predecir rpart tree
```{r}
rpart_pred <- predict(rpart_tree, val, type="class")
...
Un solo modelo
```{r}
c45_tree <- J48(labels~., data=train, control=Weka_control())
