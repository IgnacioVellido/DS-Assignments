...
Separamos un 10% para validar localmente
```{r}
...
Sería para probar con otra poda
```{r}
# Ahora podemos modificar el tamanio del arbol modificando best
# prune.trn <- prune.misclass(cart_tree, best = 4)
# plot(prune.trn)
# text(prune.trn, pretty=0)
# tree.pred <- predict(prune.trn, val, type="class")
# 
...
c45_prune <- J48(labels ~ ., data=train, control=Weka_control(R = TRUE, B=T))
