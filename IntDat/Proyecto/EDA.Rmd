...
- H.5: Viendo que contamos con un rango pequeño de años, no debería haber un cambio significativo de prestaciones entre años (explicar con plot modelYear-todas)
...
- H.10: Acceleration podría estar explicado en base a Displacement y Horse_power ??
...
scale(auto) %>%
  ...
  scatterplotMatrix(pch=20, col="deepskyblue")
```
Viendo que no son tan similares como creíamos, buscamos la fórmula real del cálculo de los caballos de vapor:
Horsepower = 	Weight / (ET/5.825)**3

Vemos que en este caso se trata de la aceleración y no la cilindrada:

```{r}
auto %>%
  mutate(hp = Weight * ((Acceleration / (234))**3) ) %>%
  # as.data.frame() %>% 
  dplyr::select(Horse_power, hp) %>%  
  scatterplotMatrix(pch=20, col="deepskyblue")
```
